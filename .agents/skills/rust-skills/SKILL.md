...
name: rust-skills
description: >
  Comprehensive Rust coding rules and guidelines with 284 entries across 27
  categories. Use when writing, reviewing, or refactoring Rust code. Covers
  ownership, error handling, async patterns, concurrency, unsafe code, API
  design, memory optimization, performance, numeric safety, conversions, serde,
  pattern matching, macros, closures, observability, testing, dependency
  management, code organization, project structure, and common anti-patterns.
  ALWAYS invoke this skill when working with Rust code, including code
  generation, review, refactoring, dependency choices, and project structure.
license: MIT
metadata:
  author: leonardomso
  version: "1.5.1"
...
If you are reading this while working on Rust code, `Cargo.toml`, workspace layout, or dependencies, you **must** have invoked the `rust-skills` skill first. If you have not invoked it, stop and call `Skill: rust-skills` now.

Non-negotiable defaults for this codebase:
- New crates: `edition = "2024"`, `rust-version = "1.85"`, workspace `resolver = "3"`.
- Before adding a dependency: read the `deps-*` rules.
- Before creating a crate: read `proj-edition-2024` and `proj-new-crate-checklist`.

---

# Rust Rules and Guidelines

Comprehensive guide for writing high-quality, idiomatic, and highly optimized Rust code. Contains 284 rules and guidelines across 27 categories, prioritized by impact to guide LLMs in code generation and refactoring. Current for Rust 1.98 (2024 edition).

## When to Apply

**This skill is mandatory for any Rust-related task.** Invoke it before writing, reviewing, refactoring, or discussing Rust code, project structure, dependencies, or tooling choices.

Reference these rules and guidelines when:
...
- Organizing modules, workspaces, or crate structure
- Adding, updating, or replacing dependencies

## Rule Severity

Every entry in this skill is labeled with a severity that tells you how strictly to follow it:

| Severity | Meaning | Default mapping from Impact |
|----------|---------|-----------------------------|
| **RULE** (MUST) | Follow it unless the project's `AGENTS.md` or a senior reviewer explicitly says otherwise. Violations are defects or review blockers. | CRITICAL, HIGH |
| **GUIDELINE** (SHOULD) | Follow by default, but you may deviate with a clear, documented reason. | MEDIUM, LOW |
| **RECOMMENDATION** (MAY) | Useful to know; apply when the situation calls for it. | REFERENCE |

The `Impact` column in the table below is the default severity. A rule file can override its own severity in its header (e.g. a HIGH-impact item marked as GUIDELINE, or a MEDIUM-impact item marked as RULE). When a project-level document such as `AGENTS.md` conflicts with this skill, the project document wins.
...
| 20 | Testing | MEDIUM | `test-` | 14 |
| 21 | Documentation | MEDIUM | `doc-` | 13 |
| 22 | Observability | MEDIUM | `obs-` | 7 |
| 23 | Performance Patterns | MEDIUM | `perf-` | 13 |
| 24 | Project Structure | HIGH | `proj-` | 21 |
| 25 | Dependencies & Crates | MEDIUM | `deps-` | 6 |
| 26 | Clippy & Linting | LOW | `lint-` | 13 |
| 27 | Anti-patterns | LOW | `anti-` | 15 |
...
- [`mem-stack-over-heap`](rules/mem-stack-over-heap.md) - Prefer the stack when the size is known and small
...
- [`api-function-design`](rules/api-function-design.md) - Keep functions focused, shallow, and easy to call
- [`api-type-design`](rules/api-type-design.md) - Favor composition and encapsulation in struct design
...
- [`async-task-owner-lifetime`](rules/async-task-owner-lifetime.md) - Long-lived async tasks must die with their owner
...
- [`conc-send-sync-bounds`](rules/conc-send-sync-bounds.md) - Apply `Send` and `Sync` trait bounds appropriately
- [`conc-crossbeam-channel`](rules/conc-crossbeam-channel.md) - Use `crossbeam-channel` for synchronous multi-producer multi-consumer message passing
...
- [`test-nextest`](rules/test-nextest.md) - Use `cargo-nextest` as a faster, more reliable test runner
- [`test-use-super`](rules/test-use-super.md) - Use `use super::*;` in inline test modules to access parent module items
...
- [`doc-comment-audience`](rules/doc-comment-audience.md) - Comment Rust-specific nuances only where they are non-obvious
...
### 24. Project Structure (HIGH)

- [`proj-thin-lib-rs`](rules/proj-thin-lib-rs.md) - Keep `lib.rs` a thin public facade
- [`proj-import-organization`](rules/proj-import-organization.md) - Keep imports explicit and ordered
- [`proj-single-responsibility`](rules/proj-single-responsibility.md) - One responsibility per function and per type
- [`proj-free-functions-vs-methods`](rules/proj-free-functions-vs-methods.md) - Prefer methods for behavior tied to one struct *(Recommended)*
- [`proj-no-pointless-reexport`](rules/proj-no-pointless-reexport.md) - Don't create re-export-only modules for a single struct
...
- [`proj-edition-2024`](rules/proj-edition-2024.md) - Use Rust 2024 edition for new projects and migrations
- [`proj-new-crate-checklist`](rules/proj-new-crate-checklist.md) - Follow the checklist when creating a new crate
- [`proj-build-rs-minimal`](rules/proj-build-rs-minimal.md) - Keep `build.rs` minimal, deterministic, and idempotent

### 25. Dependencies & Crates (MEDIUM)

- [`deps-crate-policy`](rules/deps-crate-policy.md) - Check existing crate usage before adding a new dependency
- [`deps-latest-version`](rules/deps-latest-version.md) - Use the latest stable version when adding a crate
- [`deps-read-docs`](rules/deps-read-docs.md) - Read the crate documentation for the version declared in Cargo.toml
- [`deps-respect-lock`](rules/deps-respect-lock.md) - Prefer the version already recorded in Cargo.lock
- [`deps-version-flexibility`](rules/deps-version-flexibility.md) - Prefer flexible version requirements to reduce dependency duplication
- [`deps-prefer-crate-over-handroll`](rules/deps-prefer-crate-over-handroll.md) - Prefer a small, low-overhead crate over hand-rolling when it reduces code at optimal performance
...
3. **Respect severity:** RULE (MUST) > GUIDELINE (SHOULD) > RECOMMENDATION (MAY)
4. **Prioritize** CRITICAL > HIGH > MEDIUM > LOW within the same severity
...
| Adding / changing dependencies | `deps-`, `proj-` |
| Code review | `anti-`, `lint-` |
| New crate, lib or bin | `deps-`, `proj-` |

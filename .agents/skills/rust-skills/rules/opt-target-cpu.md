...
## Benchmarks Exception

`target-cpu=native` and other `RUSTFLAGS` are useful for production builds on a known target, but they must **not** be used for benchmarks. Benchmarks must be apples-to-apples and portable so that results are reproducible across machines and commits. See the project-level benchmarking policy in [`AGENTS.md`](../AGENTS.md) for details.

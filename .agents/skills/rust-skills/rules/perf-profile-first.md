...
## Appropriate Optimizations After Profiling

Apply optimizations proportionally, only where the profile shows real time or memory being spent:

- **Algorithm / data structure**: usually the biggest win (e.g., replace a linear scan with a `HashMap` lookup).
- **Parallelization**: use `rayon` (`par_iter`) for CPU-bound, embarrassingly parallel work.
- **SIMD**: use portable SIMD or the `wide` crate for vectorizable hot loops.
- **Allocation reduction**: pre-allocate, reuse buffers, prefer `&str`/`Cow` over `String` clones.
- **Caching / memoization**: only when measurements justify the added complexity.

Avoid scattering these techniques across non-hot code; keep the rest simple.

...
## Algorithmic Efficiency Is Not Premature

This rule targets micro-optimizations made without measurements. It does not excuse picking a worse algorithm when a better one is equally clear. Choose the right data structures and asymptotic complexity up front: prefer `HashSet`/`BTreeSet` membership tests over `Vec::contains`, avoid obvious `O(n²)` loops when an `O(n)` or `O(n log n)` solution is straightforward, and keep memory usage proportional to the problem size. Reserve profiling for validating low-level changes such as SIMD, parallelization, or allocation reduction.

...
```rust
// OK: IDs that could be confused
fn transfer(from: AccountId, to: AccountId, amount: Money) { ... }

// OK: Units that shouldn't mix
struct Celsius(f64);
struct Fahrenheit(f64);

// OK: Validated strings
struct Username(String);  // Validated alphanumeric
struct Password(String);  // Never logged

// OK: Different meanings of same type
struct Milliseconds(u64);
struct Seconds(u64);

// BAD: Overkill: single use, no confusion possible
